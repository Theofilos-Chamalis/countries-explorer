...
export const getServerSideProps = async (context: GetServerSidePropsContext) => {
  ...
  const countryData = countryName ? await getCountryByNameService(countryName) : null;

  // If we don't have country data, go to the 404 page (we could also create a custom 404 page if needed)
  const notFound = !countryData || countryData?.ok === false || !countryData?.data;
...
const DetailsPage: NextPage<DetailsPageProps> = ({ countryInfo, borderCountriesNames }) => {
    ...
    return (
      <div className='flex flex-col xl:flex-row gap-2 xl:gap-10 mb-6'>
        <div className='flex flex-col gap-2'>
          ...
          <InfoRow
            label='Population'
            value={population ? new Intl.NumberFormat().format(population) : '0'}
          />
