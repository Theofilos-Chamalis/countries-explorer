...
export const getCountryByNameService = async (name: string) => {
  const countriesByNameFromBE: ApiResponse<ICountry[]> = await api.get(`/name/${name}`);
  return countriesByNameFromBE;
};

export const getCountriesByContinentService = async (continent: ContinentsType | '') => {
...
export const getCountryByCodeService = async (code: string) => {
  const countriesByCodeFromBE: ApiResponse<ICountry[]> = await api.get(`/alpha/${code}`);
